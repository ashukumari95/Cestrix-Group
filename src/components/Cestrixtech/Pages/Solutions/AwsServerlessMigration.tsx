...
import { ArrowLeft, ServerCrash, CloudLightning, ArrowRight } from "lucide-react";

export default function AwsServerlessMigration({ onNavigate }: { onNavigate?: (view: string) => void }) {
  ...
  const handleContact = () => {
    window.location.href = "mailto:info@cestrixgroup.com?subject=Urgent:%20AWS%20Migration%20Consultation";
  };
    ...
    <div className="w-full min-h-screen bg-[#F4F7FA] font-sans selection:bg-[#002D62] selection:text-white">
      ...
      <div className="bg-[#001730] text-white py-20 px-6 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative z-10 text-center">
          ...
          {/* 🔥 FIX: Top wala 'Targeted Solution' tag yahan se remove kar diya gaya hai */}
          
          <h1 className="text-4xl md:text-5xl font-bold font-heading mb-6 leading-tight mt-8">
            AWS EC2 to Serverless Migration Expert
          </h1>
          <p className="text-lg text-slate-300 font-light max-w-3xl mx-auto mb-10 leading-relaxed">
            Stop overpaying for idle EC2 servers. Resolve scaling bottlenecks and reduce cloud infrastructure billing by migrating your legacy monolith backend to a highly scalable, secure AWS Serverless architecture.
          </p>
          <button 
          ...
          >
            Request Urgent Architecture Audit <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto py-20 px-6">
        ...
        <div className="grid md:grid-cols-2 gap-10">
          ...
          <div className="bg-white p-10 rounded-xl shadow-lg border-t-4 border-red-500">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-4">The EC2 Monolith Problem</h3>
            <ul className="space-y-4 text-sm text-slate-600 font-light">
              <li>• Paying 24/7 for servers even during zero-traffic hours.</li>
              <li>• Systems crashing during sudden traffic spikes (bottleneck errors).</li>
              <li>• Developer hours wasted on server maintenance, OS patching, and load balancer setups.</li>
            </ul>
          </div>

          {/* The Fix */}
          <div className="bg-white p-10 rounded-xl shadow-lg border-t-4 border-green-500">
            <CloudLightning className="w-12 h-12 text-green-500 mb-6" />
            <h3 className="text-xl font-bold text-slate-800 mb-4">The Serverless Solution</h3>
            <ul className="space-y-4 text-sm text-slate-600 font-light">
              <li>• Zero idle costs. You only pay for exact compute milliseconds used (AWS Lambda).</li>
              <li>• Infinite auto-scaling without manual intervention.</li>
              <li>• Built-in high availability and decoupled microservices architecture.</li>
            </ul>
          </div>
        </div>
      </div>
