...
import { ArrowLeft, ServerCrash, Activity, ArrowRight } from "lucide-react";

export default function NodejsMemoryLeakFix({ onNavigate }: { onNavigate?: (view: string) => void }) {
  ...
  const handleContact = () => {
    window.location.href = "mailto:info@cestrixgroup.com?subject=Urgent:%20Node.js%20Memory%20Leak%20Diagnostics";
  };
  ...
  return (
    <div className="w-full min-h-screen bg-[#F4F7FA] font-sans selection:bg-[#002D62] selection:text-white">
      ...
      <div className="bg-[#001730] text-white py-20 px-6 relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1555066931-4365d14bab8c?auto=format&fit=crop&q=80')] opacity-10 bg-cover mix-blend-luminosity"></div>
        <div className="max-w-5xl mx-auto relative z-10 text-center">
          
          <h1 className="text-4xl md:text-5xl font-bold font-heading mb-6 leading-tight mt-8">
            Node.js Production Memory Leak & Crash Fix
          </h1>
          <p className="text-lg text-slate-300 font-light max-w-3xl mx-auto mb-10 leading-relaxed">
            Is your MERN stack backend constantly crashing under heavy load? We urgently diagnose, trace, and permanently resolve V8 engine memory leaks, event loop blockages, and API latency issues in production environments.
          </p>
          <button 
          ...
          >
            Request Urgent Diagnostics <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto py-20 px-6">
        ...
        <div className="grid md:grid-cols-2 gap-10">
          ...
          <div className="bg-white p-10 rounded-xl shadow-lg border-t-4 border-red-500">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-4">The Node.js Crash Loop</h3>
            <ul className="space-y-4 text-sm text-slate-600 font-light">
              <li>• Servers restarting multiple times a day (PM2 auto-restarts hiding the real issue).</li>
              <li>• Sluggish API response times causing timeouts and losing active user sessions.</li>
              <li>• High CPU & RAM utilization triggering massive AWS/Cloud billing alerts.</li>
            </ul>
          </div>
          ...
          <div className="bg-white p-10 rounded-xl shadow-lg border-t-4 border-green-500">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-4">The Deep-Trace Solution</h3>
            <ul className="space-y-4 text-sm text-slate-600 font-light">
              <li>• V8 Heap Snapshot analysis to pinpoint exactly which variables are hoarding memory.</li>
              <li>• Refactoring event loop blockages and optimizing asynchronous database queries.</li>
              <li>• Stabilizing your Docker/PM2 instances to guarantee 99.99% backend uptime.</li>
            </ul>
          </div>
        </div>
      </div>
